...
void LookUpGateHandler::calculate(int time, int32_t motor_ang_l, int32_t motor_ang_r, int color, int gyro, int sonic, int volt, signed char* pwm_L, signed char* pwm_R, signed int* ang_T)
{
	if(time < 0.2 * TIME_SECTION){
	...
	}else if(time < 30 * TIME_SECTION/10){
		*pwm_L = 6;
		*pwm_R = -6;
		*ang_T = TAIL_ANGLE_LUG;
	}else if(time < 50 * TIME_SECTION/10){
		*pwm_L = MOTOR_FORWARD;
		*pwm_R = MOTOR_FORWARD;
		if(color > 2 * (LIGHT_WHITE + LIGHT_BLACK) / 3){
			*pwm_L = *pwm_L - LINE_TRACE;
		...
		*ang_T = TAIL_ANGLE_LUG;
	}else if(time < 60 * TIME_SECTION/10){
		*pwm_L = 6;
		*pwm_R = -6;
		*ang_T = TAIL_ANGLE_LUG;
	}else if(time < 80 * TIME_SECTION/10){
		*pwm_L = MOTOR_FORWARD;
		*pwm_R = MOTOR_FORWARD;
		if(color > 2 * (LIGHT_WHITE + LIGHT_BLACK) / 3){
			*pwm_R = *pwm_R - LINE_TRACE;
		...
		*ang_T = tail_up[(time - 80 * TIME_SECTION/10) / (TIME_SECTION / tail_up.size())]; // DIVISION
	}
...
{
	if(time < 90 * TIME_SECTION/10) return false;
	else return true;
}
